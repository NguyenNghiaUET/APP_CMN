// Temporary: compute CRC test vectors for self-test (same algo as McuSender)
#include <cstdio>
#include <cstdint>

static const uint32_t sw_crc32_by_byte_table[256] = {
...
static uint32_t shiftBlock4(uint32_t crc) {
    crc = (crc << 8) ^ sw_crc32_by_byte_table[crc >> 24];
    crc = (crc << 8) ^ sw_crc32_by_byte_table[crc >> 24];
    crc = (crc << 8) ^ sw_crc32_by_byte_table[crc >> 24];
    crc = (crc << 8) ^ sw_crc32_by_byte_table[crc >> 24];
    return crc;
...
static uint32_t crc32_stm32(const uint8_t* data, int n) {
    uint32_t crc = 0xFFFFFFFFu;
    int i = 0;
    while (i + 4 <= n) {
        uint32_t w = ((uint32_t)data[i] << 24) | ((uint32_t)data[i+1] << 16) | ((uint32_t)data[i+2] << 8) | (uint32_t)data[i+3];
        crc ^= w;
        crc = shiftBlock4(crc);
        i += 4;
    }
    if (i < n) {
        int numOfByte = n - i;
        uint32_t last = 0;
        if (numOfByte == 1) last = (uint32_t)data[i] << 24;
        else if (numOfByte == 2) last = ((uint32_t)data[i] << 24) | ((uint32_t)data[i+1] << 16);
        else last = ((uint32_t)data[i] << 24) | ((uint32_t)data[i+1] << 16) | ((uint32_t)data[i+2] << 8);
        crc ^= last;
        crc = shiftBlock4(crc);
    }
    return crc;
...
int main() {
    uint8_t p1[] = { 0xAA, 0x00, 0x00 };
    uint8_t p2[] = { 0xAA, 0x01, 0x00, 0x5B };
    printf("CRC(AA 00 00) = 0x%08X\n", crc32_stm32(p1, 3));
    printf("CRC(AA 01 00 5B) = 0x%08X\n", crc32_stm32(p2, 4));
    return 0;
}
