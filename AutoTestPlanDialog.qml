...
Window {
    ...
    signal planSaved(string planName)
    // Signal khi lưu bài đo thành công
    ...
    title: cableName ? (isEditMode ? qsTr("Chỉnh sửa bài đo - %1").arg(
                                         cableName) : qsTr(
                                         "Tạo bài đo tự động (%1)").arg(
                                         cableName)) : (isEditMode ? qsTr(
                                                                         "Chỉnh sửa bài đo") : qsTr(
                                                                         "Tạo bài đo tự động (Cable_TKC)"))
    ...
    readonly property string selectedScriptType: selectedScriptIndex >= 0
                                                 && selectedScriptIndex < scriptModel.count ? scriptModel.get(selectedScriptIndex).scriptType : ""
    readonly property var selectedScript: selectedScriptIndex >= 0
                                          && selectedScriptIndex
                                          < scriptModel.count ? scriptModel.get(
    ...
    onScriptListRefreshChanged: _rebuildVisCache(
                                    ) // signal handler khi property ScriptListRefreshChange ,nó tự động gọ _rebuildVisCache() để cập nhật cache
    ...
    function setScriptParam(key, value) {
        if (selectedScriptIndex >= 0
                && selectedScriptIndex < scriptModel.count) {
            ...
            if (oldVal !== value) {
                if (typeof window !== "undefined" && window.addLog) {
                    ...
                    var propNames = {
                        ...
                        "pinA": "Chân A",
                        "pinB": "Chân B",
                        "portPinA": "Chân trạm A",
                        "portPinB": "Chân trạm B",
                        "labelA": "Nhãn cổng A",
                        "labelB": "Nhãn cổng B",
                        "limitLower": "Cận dưới",
                        "limitUpper": "Cận trên",
                        "numReadings": "Số lần đọc",
                        "delayBetween": "Trễ đọc",
                        "delayAfter": "Trễ chuyển mạch",
                        "sysMsg": "TB hệ thống",
                        "userMsg": "TB người dùng",
                        "unitIndex": "Đơn vị"
                    }
                    var pName = propNames[key] || key
                    window.addLog(
                                "Sửa bài đo",
                                "Đổi '" + pName + "' của [" + scriptName + "] từ '"
                                + (oldVal === undefined ? "N/A" : oldVal)
                                + "' thành '" + value + "'")
                }
    ...
    function applyDeviceConfigToAll() {
        ...
        var params = {}
        params.deviceConfig = selectedScript.deviceConfig
                !== undefined ? selectedScript.deviceConfig : false
        params.deviceSpeed = String(selectedScript.deviceSpeed || "FAST")
        params.numReadings = selectedScript.numReadings
                !== undefined ? Number(selectedScript.numReadings) : 3
        params.delayBetween = selectedScript.delayBetween
                !== undefined ? Number(selectedScript.delayBetween) : 10
        params.delayAfter = selectedScript.delayAfter
                !== undefined ? Number(selectedScript.delayAfter) : 100
        params.resistanceRange = String(selectedScript.resistanceRange
                                        || "RANGE_1KΩ")
        ...
        if (typeof keithley2110 !== "undefined" && keithley2110
            ...
            console.log("[CONFIG] ✅ Đã gửi cấu hình tới RM3544 - range:",
                        range, "speed:", speed)
        } else {
            console.log("[CONFIG] ⚠ RM3544 chưa kết nối - chỉ lưu cấu hình vào scripts")
        }

        console.log("[CONFIG] Đã áp dụng cấu hình cho", count, "scripts loại:",
                    targetTypes.join(", "))
        return count
    }
    ...
    function isSectionAllChecked(headerIndex) {
        if (headerIndex < 0 || headerIndex >= scriptModel.count)
            return true
        ...
        for (var i = headerIndex + 1; i < scriptModel.count; i++) {
            var t = String(scriptModel.get(i).scriptType || "")
            if (t === childType) {
                if (scriptModel.get(i).allowRun === false)
                    return false
            } else {
                break
            }
    ...
    function updateDisplayText() {
        ...
        if (t === "notification")
            scriptModel.setProperty(
                        selectedScriptIndex, "displayText", qsTr(
                            "Thông báo Kết nối cổng A với %1; cổng B với %2").arg(
                            item.labelA || "").arg(item.labelB || ""))
        else if (t === "continuity")
            scriptModel.setProperty(
                        selectedScriptIndex, "displayText",
                        (item.pinA !== undefined && item.pinA !== ""
                         && item.pinB !== undefined
                         && item.pinB !== "") ? qsTr("Kiểm tra thông chập giữa các chân (%1_%2–%1_%3)").arg(item.labelA || "").arg(item.pinA || "").arg(item.pinB || "") : qsTr("Kiểm tra thông chập giữa các chân (%1 <-> %2)").arg(
                                                    item.labelA || "").arg(
                                                    item.labelB || ""))
        else if (t === "sheath_insulation")
            scriptModel.setProperty(
                        selectedScriptIndex, "displayText", qsTr(
                            "Kiểm tra điện trở cách điện với vỏ [%1_%2 <-> Cable cover]").arg(
                            item.labelA || "").arg(item.pinA || ""))
    }
    function update() {
        if (selectedScriptIndex < 0 || selectedScriptIndex >= scriptModel.count)
            return
    ...
    function serializeScriptModel() {
        ...
        for (var i = 0; i < scriptModel.count; i++) {
            ...
            var ob = {
                ...
                "pinA": String(item.pinA || ""),
                "labelB": String(item.labelB || ""),
                "pinB": String(item.pinB || ""),
                "portLabelA": String(item.portLabelA || ""),
                "portLabelB": String(item.portLabelB || ""),
                "allowRun": item.allowRun !== undefined ? Boolean(
                                                              item.allowRun) : true,
                "stopOnFail": item.stopOnFail !== undefined ? Boolean(
                                                                  item.stopOnFail) : true,
                "unitIndex": item.unitIndex !== undefined ? Number(
                                                                item.unitIndex) : 0,
                "limitLower": item.limitLower !== undefined ? Number(
                                                                  item.limitLower) : 0,
                "limitUpper": item.limitUpper !== undefined ? Number(
                                                                  item.limitUpper) : ((item.scriptType === "continuity" || item.scriptType === "sheath_insulation") ? 999999 : 0),
                "numReadings": item.numReadings !== undefined ? Number(
                                                                    item.numReadings) : 3,
                "delayBetween": item.delayBetween !== undefined ? Number(
                                                                      item.delayBetween) : 10,
                "delayAfter": item.delayAfter !== undefined ? Number(
                                                                  item.delayAfter) : 100,
                "dischargeDelay": item.dischargeDelay !== undefined ? Number(
                                                                          item.dischargeDelay) : 0,
                "sysMsg": String(item.sysMsg || ""),
                "userMsg": String(item.userMsg || ""),
                "deviceConfig": item.deviceConfig !== undefined ? Boolean(
                                                                      item.deviceConfig) : false,
                "resistanceRange": String(item.resistanceRange || "RANGE_1KΩ"),
                "deviceSpeed": String(item.deviceSpeed || "FAST"),
                "measureVoltage": item.measureVoltage
                                  !== undefined ? Number(
                                                      item.measureVoltage) : 250,
                "currentLimit": String(item.currentLimit || "Auto"),
                "trigDelay": item.trigDelay !== undefined ? Number(
                                                                item.trigDelay) : 1,
                "avgCount": item.avgCount !== undefined ? Number(
                                                              item.avgCount) : 1,
                "avgEnabled": item.avgEnabled !== undefined ? Boolean(
                                                                  item.avgEnabled) : false
            }
            _addPortPinsIfValid(ob, item)
            if (ob.scriptType === "continuity_header"
                    || ob.scriptType === "sheath_insulation_header")
    ...
    function _addPortPinsIfValid(ob, item) {
        ...
        if (item.portPinA !== undefined && item.portPinA !== null && !isNaN(
                    Number(item.portPinA))) {
            var portA = Number(
                        item.portPinA) // chuyển đổi portPinA thành số nguyên
            if (portA >= 0) {
                ob.portPinA = portA // gán portPinA cho ob
            }
        }
        if (item.portPinB !== undefined && item.portPinB !== null && !isNaN(
                    Number(item.portPinB))) {
            var portB = Number(
                        item.portPinB) // chuyển đổi portPinB thành số nguyên
            if (portB >= 0) {
    ...
    function openWithCable(name, tableRows) {
        // mở cửa sổ tạo bài đo tự động
        isEditMode = false
        cableName = name || "" // CableName tên cáp
        show() // mở cửa sổ
        Qt.callLater(function () {
            // Đợi 1 chút để cửa sổ hiện lên rồi mới build script
    ...
    Timer {
        ...
        onTriggered: {
            var batchSize = 80
            var end = Math.min(autoTestPlanDialog._batchIndex + batchSize,
                               autoTestPlanDialog._pendingScripts.length)
    ...
    function _defaultParams(o) {
        var d = {
            ...
            "labelA": o.labelA || "",
            "pinA": o.pinA || "",
            "labelB"// Thông chập cùng giắc: nếu labelB rỗng thì lấy labelA
            : (o.labelB
               || "") !== "" ? (o.labelB
                                || "") : (o.scriptType === "continuity" ? (o.labelA
                                                                           || "") : ""),
            "pinB": o.pinB || "",
            "portLabelA": o.portLabelA || "",
            "portLabelB": o.portLabelB || "",
            ...
            "limitUpper": (function () {
                var def = (o.scriptType === "continuity"
                           || o.scriptType === "sheath_insulation") ? 999999 : 0
        ...
        if (o.portPinA !== undefined && o.portPinA !== null && !isNaN(
                    Number(o.portPinA))) {
        ...
        if (o.portPinB !== undefined && o.portPinB !== null && !isNaN(
                    Number(o.portPinB))) {
    ...
    function _isHeaderRow(row) {
        ...
        return (c0 === qsTr("Nhãn giắc đầu A") && c3 === qsTr(
                    "Nhãn giắc đầu B")) || c1 === qsTr("Tên chân đầu A")
    }
    ...
    function buildScriptListFromExcel(rows) {
        ...
        scriptModel.clear()
        scriptModel.append({
                               "displayText": qsTr("Khởi tạo hệ thống"),
                               "scriptType": "system_init"
                           })
        var a = buildScriptArrayFromExcel(rows)
        for (var j = 0; j < a.length; j++)
            scriptModel.append(a[j])
        _rebuildVisCache()
        Qt.callLater(function () {
            if (typeof scriptListView !== "undefined" && scriptListView) {
                scriptListView.model = scriptModel
    ...
    function _buildScriptListFromExcelLegacy(rows) {
        ...
        while (i < rows.length) {
            ...
            for (var gx = 0; gx < group.length; gx++) {
                ...
                if (isNaN(portA))
                    portA = parseInt(pa, 10)
                if (pa !== "" && pinsA.findIndex(function (x) {
                    return x.pin === pa
                }) < 0)
                    pinsA.push({
                                   "pin": pa,
                ...
                if (isNaN(portB))
                    portB = parseInt(pb, 10)
                if (pb !== "" && pinsB.findIndex(function (x) {
                    return x.pin === pb
                }) < 0)
                    pinsB.push({
                                   "pin": pb,
            ...
            for (var ia = 0; ia < pinsA.length; ia++) {
                for (var ja = ia + 1; ja < pinsA.length; ja++) {
                    var contA = {
                        "displayText": qsTr("Kiểm tra thông chập giữa các chân (%1_%2–%1_%3)").arg(
                                           labelA).arg(pinsA[ia].pin).arg(
                                           pinsA[ja].pin),
                        ...
                        "pinA": pinsA[ia].pin,
                        "pinB": pinsA[ja].pin,
                        "portPinA": pinsA[ia].port,
                        "portPinB": pinsA[ja].port
                    }
            ...
            for (var ib = 0; ib < pinsB.length; ib++) {
                for (var jb = ib + 1; jb < pinsB.length; jb++) {
                    var contB = {
                        ...
                        "pinA": pinsB[ib].pin,
                        "pinB": pinsB[jb].pin,
                        "portPinA": pinsB[ib].port,
                        "portPinB": pinsB[jb].port
                    }
            ...
            for (var s = 0; s < pinsA.length; s++) {
                scriptModel.append(_defaultParams({
                                                      "displayText": qsTr("Kiểm tra điện trở cách điện với vỏ [%1_%2 <-> Cable cover]").arg(
                                                                         labelA).arg(
                                                                         pinsA[s].pin),
    ...
    function buildScriptArrayFromExcel(rows) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (!inSheathSection && c0.indexOf("đo điện trở") >= 0
                    || c0.indexOf("Đo điện trở") >= 0) {
                if (c1 === "" && c2 === "") {
                    // xác nhận là dòng header, không phải data
            ...
            if (!inSheathSection) {
                ...
                if (isNaN(limitUp) || limitUp <= 0)
                    limitUp = 0.8

                arr.push(_defaultParams({
                                            "displayText": qsTr("Thông chập [%1_%2 → %3_%4]").arg(
                                                               c0).arg(c1).arg(
                                                               c3).arg(c4),
                                            "scriptType": "continuity",
                                            "labelA": c0,
                ...
                if (isNaN(limitLow) || limitLow <= 0)
                    limitLow = 100

                arr.push(_defaultParams({
                                            "displayText": qsTr(
                                                               "Cách điện vỏ [%1_%2 → Ground]").arg(
                                                               c0).arg(c1),
                                            "scriptType": "sheath_insulation",
                                            "labelA": c0,
                                            "pinA": c1,
    ...
    function buildScriptListFromTemplate() {
        ...
        var notT = cableName ? qsTr("Thông báo Kết nối cổng A với %1; cổng B với ...").arg(
                                   cableName) : qsTr(
                                   "Thông báo Kết nối cổng A với ...; cổng B với ...")
        scriptModel.append(_defaultParams({
                                              "displayText": notT,
                                              "scriptType": "notification",
                                              "labelA": cableName || "",
        ...
        scriptModel.append(_defaultParams({
                                              "displayText": qsTr("Kiểm tra thông chập giữa các chân [Connector_pin1 <-> Connector_pin2]"),
                                              "scriptType": "continuity",
                                              "labelA": "",
                                              "pinA": "",
                                              "pinB": "",
        ...
        scriptModel.append(_defaultParams({
                                              "displayText": qsTr("Kiểm tra điện trở cách điện với vỏ [X <-> Cable cover]"),
                                              "scriptType": "sheath_insulation",
                                              "labelA": "",
                                              "pinA": "",
                                              "labelB": "Cable cover",
    ...
    ListModel {
        ...
        ListElement {
            displayText: qsTr("Hiện thông báo")
            scriptType: "notification"
        }
        ListElement {
            displayText: qsTr("Thiết lập rơ le")
            scriptType: "relay"
        }
        ListElement {
            displayText: qsTr("Lưu kết quả đo")
        ...
        ListElement {
            displayText: qsTr("➕ Thêm cặp cổng mới")
            scriptType: "add_connector_pair"
        }
    }
    ...
    function insertTemplateScript(scriptType, atIndex, templateDisplayText) {
        if (typeof window !== "undefined" && window.addLog) {
            window.addLog(
                        "Sửa bài đo",
                        "Chèn script mới: " + (templateDisplayText
                                               || scriptType) + " tại vị trí " + (atIndex + 1))
        }
        ...
        if (t === "system_init") {
            ...
            displayText = displayText
                    || (cableName ? qsTr("Thông báo Kết nối cổng A với %1; cổng B với ...").arg(
                                        cableName) : qsTr(
                                        "Thông báo Kết nối cổng A với ...; cổng B với ..."))
            ...
            params = _defaultParams({
                                        "displayText": displayText || qsTr(
                                                           "Kiểm tra thông chập giữa các chân [Connector_pin1 <-> Connector_pin2]"),
                                        "scriptType": "continuity",
                                        "labelA": "",
                                        "pinA": "",
                                        "pinB": "",
            ...
            params = _defaultParams({
                                        "displayText": displayText || qsTr(
                                                           "Kiểm tra điện trở cách điện với vỏ [X <-> Cable cover]"),
                                        "scriptType": "sheath_insulation",
                                        "labelA": "",
                                        "pinA": "",
                                        "labelB": "Cable cover",
    ...
    function buildScriptsForConnectorPair(labelA, labelB, pinsA, pinsB, insertIndex) {
        var arr = []
        // 1. Notification
        var notMsg = qsTr("Thông báo Kết nối cổng A với %1; cổng B với %2").arg(
                    labelA).arg(labelB)
        arr.push(_defaultParams({
                                    "displayText": notMsg,
                                    "scriptType": "notification",
        ...
        for (var c = 0; c < maxLen; c++) {
            ...
            arr.push(_defaultParams({
                                        "displayText": qsTr("Kiểm tra thông chập giữa các chân (%1_%2–%3_%4)").arg(
                                                           labelA).arg(
                                                           cPinA.pin).arg(
                                                           labelB).arg(
                                                           cPinB.pin),
        ...
        for (var sb = 0; sb < pinsB.length; sb++)
            arr.push(_defaultParams({
                                        ...
                                        "labelB": "Cable cover",
                                        "pinB": "",
                                        "portPinA": pinsB[sb].port
                                    }))

        // Insert tất cả vào scriptModel
        var idx = insertIndex >= 0 ? insertIndex : scriptModel.count
        for (var k = 0; k < arr.length; k++)
            scriptModel.insert(idx + k, arr[k])
        selectedScriptIndex = idx
        scriptListRefresh++
        _rebuildVisCache()
        if (typeof window !== "undefined" && window.addLog)
            window.addLog("Sửa bài đo", "Đã thêm cặp cổng " + labelA + " ↔ "
                          + labelB + " (" + arr.length + " scripts)")
        return arr.length
    }

    function deleteScriptAt(index) {
        if (index < 0 || index >= scriptModel.count)
            return
        if (typeof window !== "undefined" && window.addLog) {
            window.addLog(
                        "Sửa bài đo",
                        "Xóa script: " + (scriptModel.get(index).displayText
                                          || "Script") + " tại vị trí " + (index + 1))
        }
        scriptModel.remove(index)
        if (selectedScriptIndex >= scriptModel.count)
            selectedScriptIndex = scriptModel.count - 1
        else if (selectedScriptIndex >= index && selectedScriptIndex > 0)
            selectedScriptIndex--
        scriptListRefresh++
    }
    ...
    function copyScriptAt(index) {
        ...
        if (typeof window !== "undefined" && window.addLog) {
            window.addLog("Sửa bài đo",
                          "Copy script: " + (item.displayText || "Script"))
        }
        copiedScript = {}
        for (var k in item)
            copiedScript[k] = item[k]
    }
    ...
    function pasteScriptAt(index) {
        ...
        if (typeof window !== "undefined" && window.addLog) {
            window.addLog(
                        "Sửa bài đo",
                        "Paste script: " + (copiedScript.displayText
                                            || "Script") + " vào vị trí " + (index + 2))
        }
    ...
    function moveScriptAt(fromIndex, direction) {
        if (fromIndex < 0 || fromIndex >= scriptModel.count)
            return
        var toIndex = direction === "up" ? fromIndex - 1 : fromIndex + 1
        if (toIndex < 0 || toIndex >= scriptModel.count)
            return

        if (typeof window !== "undefined" && window.addLog) {
            window.addLog("Sửa bài đo",
                          "Di chuyển script: '" + (scriptModel.get(
                                                       fromIndex).displayText
                                                   || "Script") + "' từ "
                          + (fromIndex + 1) + " đến " + (toIndex + 1))
        }
    ...
    function moveSelectedScriptsTo(targetIdx) {
        if (selectedIndices.length === 0)
            return
        if (typeof window !== "undefined" && window.addLog) {
            window.addLog("Sửa bài đo", "Di chuyển " + selectedIndices.length
                          + " script đến vị trí " + (targetIdx + 1))
        }
        ...
        for (var m = 0; m < sorted.length; m++) {
            if (sorted[m] < targetIdx)
                adjustedTarget--
        }
        if (adjustedTarget < 0)
            adjustedTarget = 0
        var maxPos = scriptModel.count - blockSize
        if (adjustedTarget > maxPos)
            adjustedTarget = maxPos
        ...
        if (isContiguous) {
            ...
            for (var j = 0; j < allItems.length; j++) {
                if (selSet[j])
                    ...
                    remainingItems.push(allItems[j])
            }
            var at = adjustedTarget
            if (at > remainingItems.length)
                at = remainingItems.length
            var newOrder = remainingItems.slice(0, at).concat(
                        selectedItems).concat(remainingItems.slice(at))
            scriptModel.clear()
    ...
    function deleteSelectedScripts() {
        ...
        if (scriptModel.count > 0) {
            selectedScriptIndex = Math.min(sorted[sorted.length - 1],
                                           scriptModel.count - 1)
                    ...
                    QC.ScrollView {
                        ...
                        ListView {
                            ...
                            delegate: QC.ItemDelegate {
                                ...
                                onClicked: insertTemplateScript(
                                               model.scriptType,
                                               autoTestPlanDialog.selectedScriptIndex
                                               >= 0 ? autoTestPlanDialog.selectedScriptIndex
                                                      + 1 : scriptModel.count,
                                               model.displayText)
                            }
                        }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        QC.ComboBox {
                            ...
                            onCurrentIndexChanged: {
                                if (currentIndex >= 0
                                        && currentIndex < editPlanNames.length
                                        && currentIndex !== editPlanIndex) {
                                    editPlanIndex = currentIndex
                                    loadPlanIntoModel(
                                                editPlanNames[currentIndex])
                                    selectedScriptIndex = scriptModel.count > 0 ? 0 : -1
                                }
                            }
                        }
                    ...
                    RowLayout {
                        ...
                        QC.CheckBox {
                            ...
                            onToggled: autoTestPlanDialog.toggleAllScripts(
                                           checked)
                            padding: 0
                            indicator.width: 16
                            indicator.height: 16
                        ...
                        QC.Label {
                            text: autoTestPlanDialog._isLoading ? qsTr("Scripts (%1/%2)").arg(
                                                                      scriptModel.count).arg(
                                                                      autoTestPlanDialog._pendingScripts.length) : qsTr(
                                                                      "Scripts (%1)").arg(
                                                                      scriptModel.count)
                            font.bold: true
                            font.pixelSize: 14
                        ...
                        Rectangle {
                            ...
                            border.color: "#90CAF9"
                            radius: 3
                            visible: !autoTestPlanDialog._isLoading
                                     && scriptModel.count > 0
                            QC.Label {
                                id: pairCountLabel
                            ...
                            delegate: Item {
                                ...
                                height: {
                                    ...
                                    if (t === "notification"
                                            ...
                                            || t === "save_result"
                                            || t === "continuity_header"
                                            || t === "sheath_insulation_header")
                                        return 32
                                    // Children: kiểm tra header cha có expanded không
                                    if (t === "continuity"
                                            || t === "sheath_insulation") {
                                        if (!autoTestPlanDialog._isSectionChildVisible(
                                                    t, index))
                                            return 0
                                    }
                                    return 32
                                ...
                                property bool isHeader: String(model.scriptType
                                                               || "").indexOf(
                                                            "_header") >= 0
                                ...
                                Rectangle {
                                    anchors.fill: parent
                                    radius: 2
                                    color: scriptDelegate.isHeader ? (scriptDelegate.isSelected ? "#BBDEFB" : "#E8EAF6") : (scriptDelegate.isSelected ? "#BBDEFB" : (scriptListView.currentIndex === index ? "#E3F2FD" : "transparent"))
                                        ...
                                        QC.CheckBox {
                                            checked: {
                                                if (scriptDelegate.isHeader) {
                                                    var _ = autoTestPlanDialog.scriptListRefresh
                                                    return autoTestPlanDialog.isSectionAllChecked(
                                                                index)
                                                }
                                            ...
                                            onToggled: {
                                                if (scriptDelegate.isHeader) {
                                                    autoTestPlanDialog.toggleSectionScripts(
                                                                index, checked)
                                                } else {
                                                    scriptModel.setProperty(
                                                                index,
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: function (mouse) {
                                            if (mouse.button === Qt.RightButton) {
                                                ...
                                                if (!(autoTestPlanDialog.selectedIndexMap[index]
                                                      === true)) {
                                                    autoTestPlanDialog.selectedScriptIndex = index
                                                    autoTestPlanDialog.setSelection(
                                                                [index])
                                                }
                                                autoTestPlanDialog.openScriptContextMenu(
                                                            index)
                                                return
                                            }

                                            // Header toggle
                                            if (model.scriptType === "continuity_header"
                                                    || model.scriptType
                                                    === "sheath_insulation_header") {
                                                var exp = scriptModel.get(
                                            ...
                                            if (mouse.modifiers & Qt.ControlModifier) {
                                                ...
                                                autoTestPlanDialog.setSelection(
                                                            arr)
                                                autoTestPlanDialog.selectedScriptIndex = arr.length
                                        ...
                                        onPositionChanged: function (mouse) {
                                            if (pressed && Math.abs(
                                                ...
                                                var mappedY = scriptDelegate.mapToItem(
                                                            scriptListView,
                                                            mouse.x, mouse.y).y
                                                var targetI = scriptListView.indexAt(
                                                            0,
                                                            mappedY + scriptListView.contentY)
                                                if (targetI < 0)
                                                    targetI = scriptModel.count
                                        ...
                                        onReleased: function (mouse) {
                                            if (dragging
                                                    && autoTestPlanDialog.dropTargetIndex >= 0) {
                                                var fromIdx = autoTestPlanDialog.dragFromIndex
                                                if (fromIdx < 0
                                                        || fromIdx >= scriptModel.count) {
                                                    dragging = false
                                                ...
                                                if (fromType.indexOf(
                                                    ...
                                                    var childType = fromType.replace(
                                                                "_header", "")
                                                    var groupStart = fromIdx
                                                    var groupEnd = fromIdx // inclusive
                                                    for (var ci = fromIdx + 1; ci
                                                         < scriptModel.count; ci++) {
                                                        if (String(scriptModel.get(
                                                                       ci).scriptType
                                                                   || "") === childType) {
                                                            groupEnd = ci
                                                        } else {
                                                    ...
                                                    if (dropTarget >= groupStart
                                                            && dropTarget <= groupEnd + 1) {
                                                        dragging = false
                                                        autoTestPlanDialog.dragFromIndex = -1
                                                        autoTestPlanDialog.dropTargetIndex = -1
                                                        return
                                                    ...
                                                    // Di chuyển block
                                                    autoTestPlanDialog.moveSelectedScriptsTo(
                                                                dropTarget)
                                                } else {
                                                    // === Kéo item thường ===
                ...
                ColumnLayout {
                    ...
                    QC.GroupBox {
                        ...
                        GridLayout {
                            ...
                            QC.Label {
                                text: qsTr("Cho phép chạy")
                            }
                            QC.CheckBox {
                                id: allowRunCheck
                                checked: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.allowRun : true
                                Layout.leftMargin: 4
                                onToggled: autoTestPlanDialog.setScriptParam(
                                               "allowRun", checked)
                            }
                            QC.Label {
                                text: qsTr("Dừng khi Không đạt")
                            }
                            QC.CheckBox {
                                id: stopOnFailCheck
                            ...
                            QC.TextField {
                                ...
                                onTextChanged: autoTestPlanDialog.setScriptParam(
                                                   "displayText", text)
                            }
                        }
                    }
                    QC.GroupBox {
                        title: selectedScriptType
                               === "notification" ? qsTr("Test Parameter") : qsTr(
                                                        "Thông số bài đo")
                        Layout.fillWidth: true
                        Layout.preferredWidth: 1
                        visible: selectedScriptType !== ""
                                 && selectedScriptType !== "system_init"
                        ColumnLayout {
                            width: parent.width - 40
                            Loader {
                                id: paramLoader
                                Layout.fillWidth: true
                                sourceComponent: selectedScriptType === "notification" ? notificationParams : selectedScriptType === "continuity" ? measurementParams : selectedScriptType === "sheath_insulation" ? sheathParams : emptyParams
                            }
                        }
                    }
                    QC.GroupBox {
                        ...
                        Layout.fillWidth: true
                        Layout.preferredWidth: 1
                        visible: selectedScriptType === "continuity"
                                 || selectedScriptType === "sheath_insulation"
                        ColumnLayout {
                            width: parent.width - 40
                            Loader {
                                id: deviceParamLoader
                                Layout.fillWidth: true
                                sourceComponent: (selectedScriptType === "continuity"
                                                  || selectedScriptType === "sheath_insulation") ? deviceParamsResistance : emptyParams
                            }
                            ...
                            Rectangle {
                                ...
                                RowLayout {
                                    ...
                                    QC.Button {
                                        ...
                                        Layout.fillWidth: true
                                        onClicked: {
                                            if (!autoTestPlanDialog.selectedScript)
                                                return
                                            var st = String(
                                                        autoTestPlanDialog.selectedScript.scriptType
                                                        || "")
                                            if (st === "continuity"
                                                    || st === "sheath_insulation") {
                                                if (typeof keithley2110 !== "undefined"
                                                        && keithley2110
                                                        && keithley2110.isOpen) {
                                                    var range = String(
                                                                autoTestPlanDialog.selectedScript.resistanceRange
                                                                || "RANGE_1KΩ")
                                                    var speed = String(
                                                                autoTestPlanDialog.selectedScript.deviceSpeed
                                                                || "FAST")
                                                    keithley2110.configureRM3544(
                                                                range, speed)
                                                    applyAllResult.text = qsTr(
                                                                "✅ Đã cấu hình RM3544 - range: %1, speed: %2").arg(
                                                                range).arg(
                                                                speed)
                                    ...
                                    QC.Button {
                                        id: applyAllBtn
                                        text: qsTr(
                                                  "📋 Áp dụng cho tất cả Thông chập + Cách điện vỏ")
                                        Layout.fillWidth: true
                                        onClicked: {
                                            ...
                                            if (count > 0) {
                                                ...
                                                deviceConnected
                                                        = (typeof keithley2110 !== "undefined"
                                                           && keithley2110
                                                           && keithley2110.isOpen)
                                                if (deviceConnected) {
                                                    applyAllResult.text = qsTr(
                                                                "✅ Đã áp dụng cho %1 scripts + cấu hình máy đo").arg(
                                                                count)
                                                    applyAllResult.color = "#2E7D32"
                                                } else {
                                                    applyAllResult.text = qsTr(
                                                                "✅ Đã áp dụng cho %1 scripts (máy đo chưa kết nối)").arg(
                                                                count)
                                                    applyAllResult.color = "#E65100"
                                                }
        ...
        RowLayout {
            ...
            QC.Button {
                ...
                onClicked: {
                    ...
                    if (name && typeof testPlanManager !== "undefined"
                            && testPlanManager) {
                        if (testPlanManager.saveTestPlan(name, jsonStr)) {
                            ...
                            if (typeof window !== "undefined")
                                window.addLog(
                                            "Sửa bài đo",
                                            "Đã lưu tất cả thay đổi vào bài đo: " + name)
    ...
    Component {
        ...
        GridLayout {
            ...
            QC.ComboBox {
                ...
                Layout.fillWidth: true
                onCurrentIndexChanged: autoTestPlanDialog.setScriptParam(
                                           "unitIndex", currentIndex)
            }
            QC.Label {
                text: qsTr("Giá trị giới hạn đạt dưới (Ω)")
            }
            QC.TextField {
                id: limitLowerField
                Layout.fillWidth: true
                text: {
                    if (autoTestPlanDialog.selectedScriptType === "continuity")
                        return "1000"
                    if (autoTestPlanDialog.selectedScript)
                        return String(
                                    autoTestPlanDialog.selectedScript.limitLower)
                    return "0"
                }
                ...
                onEditingFinished: {
                    ...
                    if (!isNaN(val))
                        autoTestPlanDialog.setScriptParam("limitLower", val)
                }
            }
            QC.Label {
                text: qsTr("Giá trị giới hạn đạt trên (Ω)")
            }
            QC.TextField {
                id: limitUpperField
                Layout.fillWidth: true
                ...
                text: {
                    ...
                    if (autoTestPlanDialog.selectedScript)
                        return String(
                                    autoTestPlanDialog.selectedScript.limitUpper)
                    return "0.8"
                }
                validator: DoubleValidator {
                    bottom: 0
                ...
                onEditingFinished: {
                    var val = parseFloat(text)
                    if (!isNaN(val))
                        autoTestPlanDialog.setScriptParam("limitUpper", val)
                }
            }
            QC.Label {
                text: qsTr("Nhãn giắc đầu A")
            }
            QC.TextField {
                Layout.fillWidth: true
                text: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.labelA : ""
            ...
            QC.Label {
                text: qsTr("Tên chân đầu A")
            }
            QC.TextField {
                Layout.fillWidth: true
                text: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.pinA : ""
                onTextChanged: {
                    autoTestPlanDialog.setScriptParam("pinA", text)
                    autoTestPlanDialog.updateDisplayText()
                }
            }
            QC.Label {
                text: qsTr("Tên chân đầu B")
            }
            QC.TextField {
                Layout.fillWidth: true
                text: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.pinB : ""
                onTextChanged: {
                    autoTestPlanDialog.setScriptParam("pinB", text)
                    autoTestPlanDialog.updateDisplayText()
                }
            }
            QC.Label {
                text: qsTr("Chân đo cổng A")
            }
            QC.SpinBox {
                id: portPinASpin
                editable: true
                value: autoTestPlanDialog.selectedScript
                       && autoTestPlanDialog.selectedScript.portPinA !== undefined
                       && !isNaN(
                           Number(autoTestPlanDialog.selectedScript.portPinA))
                       && Number(autoTestPlanDialog.selectedScript.portPinA)
                       >= 0 ? Number(
                                  autoTestPlanDialog.selectedScript.portPinA) : -1
                from: -1
                to: 255
                Layout.fillWidth: true
                onValueChanged: {
            ...
            QC.Label {
                text: qsTr("Chân đo cổng B")
            }
            QC.SpinBox {
                id: portPinBSpin
                editable: true
                value: autoTestPlanDialog.selectedScript
                       && autoTestPlanDialog.selectedScript.portPinB !== undefined
                       && !isNaN(
                           Number(autoTestPlanDialog.selectedScript.portPinB))
                       && Number(autoTestPlanDialog.selectedScript.portPinB)
                       >= 0 ? Number(
                                  autoTestPlanDialog.selectedScript.portPinB) : -1
                from: -1
                to: 255
                Layout.fillWidth: true
                onValueChanged: {
            ...
            QC.Label {
                text: qsTr("Số lần đọc kết quả đo")
            }
            QC.SpinBox {
                editable: true
                ...
                to: 100
                Layout.fillWidth: true
                onValueModified: autoTestPlanDialog.setScriptParam(
                                     "numReadings", value)
            }
            QC.Label {
                text: qsTr("Trễ giữa các lần đọc (ms)")
            }
            QC.SpinBox {
                editable: true
            ...
            QC.Label {
                text: qsTr("Trễ sau khi chuyển mạch (ms)")
            }
            QC.SpinBox {
                editable: true
    ...
    Component {
        ...
        GridLayout {
            ...
            QC.TextField {
                ...
                Layout.fillWidth: true
                text: autoTestPlanDialog.selectedScript ? String(
                                                              autoTestPlanDialog.selectedScript.limitLower) : "1000"
                validator: DoubleValidator {
                    bottom: 0
                    top: 999999
                    decimals: 6
            ...
            QC.Label {
                text: qsTr("Nhãn giắc đầu đo (không phải GND)")
            }
            QC.TextField {
                Layout.fillWidth: true
                text: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.labelA : ""
    ...
    Component {
        ...
        GridLayout {
            ...
            columnSpacing: 12
            QC.Label {
                text: qsTr("Cấu hình máy đo")
            }
            QC.CheckBox {
                checked: autoTestPlanDialog.selectedScript ? autoTestPlanDialog.selectedScript.deviceConfig : false
                Layout.leftMargin: 4
                onToggled: autoTestPlanDialog.setScriptParam("deviceConfig",
                                                             checked)
            }
            QC.Label {
                text: qsTr("Dải đo điện trở")
            }
            QC.ComboBox {
            ...
            QC.ComboBox {
                ...
                Layout.fillWidth: true
                onCurrentIndexChanged: autoTestPlanDialog.setScriptParam(
                                           "deviceSpeed", model[currentIndex])
            }
        }
    }
    ...
    Component {
        ...
        GridLayout {
            ...
            QC.SpinBox {
                ...
                Layout.fillWidth: true
                onValueModified: autoTestPlanDialog.setScriptParam(
                                     "measureVoltage", value)
            }
            QC.Label {
                text: qsTr("Giới hạn dòng đo")
            }
            QC.ComboBox {
                ...
                Layout.fillWidth: true
                onCurrentIndexChanged: autoTestPlanDialog.setScriptParam(
                                           "currentLimit", model[currentIndex])
            }
            QC.Label {
                text: qsTr("Tốc độ máy đo")
            }
            ...
            QC.SpinBox {
                editable: true
                value: autoTestPlanDialog.selectedScript ? (autoTestPlanDialog.selectedScript.trigDelay !== undefined ? autoTestPlanDialog.selectedScript.trigDelay : 1) : 1
                from: 0
                to: 999
                Layout.fillWidth: true
                onValueModified: autoTestPlanDialog.setScriptParam("trigDelay",
                                                                   value)
                QC.ToolTip.visible: hovered
            ...
            QC.CheckBox {
                checked: autoTestPlanDialog.selectedScript ? (autoTestPlanDialog.selectedScript.avgEnabled !== undefined ? autoTestPlanDialog.selectedScript.avgEnabled : false) : false
                Layout.leftMargin: 4
                text: qsTr("Bật Average")
                onToggled: autoTestPlanDialog.setScriptParam("avgEnabled",
                                                             checked)
                QC.ToolTip.visible: hovered
            ...
            QC.Label {
                text: qsTr("Số lần AVG")
            }
            QC.SpinBox {
                editable: true
                value: autoTestPlanDialog.selectedScript ? (autoTestPlanDialog.selectedScript.avgCount !== undefined ? autoTestPlanDialog.selectedScript.avgCount : 1) : 1
                from: 1
                to: 100
                Layout.fillWidth: true
                onValueModified: autoTestPlanDialog.setScriptParam("avgCount",
                                                                   value)
                QC.ToolTip.visible: hovered
    ...
    FileDialog {
        ...
        onAccepted: {
            ...
            if (path && typeof cableListManager !== "undefined"
                    && cableListManager) {
                var rows = cableListManager.loadTableData(path)
                var idx = scriptContextMenuIndex >= 0 ? scriptContextMenuIndex
                                                        + 1 : scriptModel.count
    ...
    QC.Menu {
        ...
        Repeater {
            ...
            delegate: QC.MenuItem {
                text: model.displayText
                onTriggered: insertTemplateScript(
                                 model.scriptType,
                                 (autoTestPlanDialog.scriptContextMenuIndex
                                  >= 0 ? autoTestPlanDialog.scriptContextMenuIndex
                                         + 1 : scriptModel.count),
                                 model.displayText)
            }
        }
        QC.MenuSeparator {}
        QC.MenuItem {
            text: autoTestPlanDialog.selectedIndices.length
                  > 1 ? qsTr("Xóa %1 bài đo đã chọn").arg(
                            autoTestPlanDialog.selectedIndices.length) : qsTr(
                            "Xóa bài đo")
            onTriggered: {
        ...
        QC.MenuItem {
            text: qsTr("Dán")
            onTriggered: pasteScriptAt(
                             scriptContextMenuIndex
                             >= 0 ? scriptContextMenuIndex + 1 : scriptModel.count)
        }
    }
    ...
    QC.Dialog {
        ...
        function _autoAddRow() {
            if (pinRowsModel.count === 0)
                return
            var last = pinRowsModel.get(pinRowsModel.count - 1)
            if (String(last.pinNameA || "").trim() !== ""
                    || String(last.portMcuA || "").trim() !== "" || String(
                        last.pinNameB || "").trim() !== ""
                    || String(last.portMcuB || "").trim() !== "") {
                pinRowsModel.append({
                                        "pinNameA": "",
                                        "portMcuA": "",
                                        "pinNameB": "",
        ...
        contentItem: ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        QC.TextField {
                            ...
                            background: Rectangle {
                                radius: 5
                                color: "white"
                                border.color: labelAField2.activeFocus ? "#1565C0" : "#E0E0E0"
                                border.width: labelAField2.activeFocus ? 2 : 1
                            }
                        }
                    }
                }
                    ...
                    RowLayout {
                        ...
                        QC.TextField {
                            ...
                            background: Rectangle {
                                radius: 5
                                color: "white"
                                border.color: labelBField2.activeFocus ? "#2E7D32" : "#E0E0E0"
                                border.width: labelBField2.activeFocus ? 2 : 1
                            }
                        }
                    }
                }
            ...
            Rectangle {
                ...
                Row {
                    ...
                    spacing: 0
                    Item {
                        width: addConnectorPairDialog.colStt
                    ...
                    Rectangle {
                        width: addConnectorPairDialog.colPinA
                    ...
                    Rectangle {
                        width: addConnectorPairDialog.colPortA
                        height: parent.height - 4
                        anchors.verticalCenter: parent.verticalCenter
                        radius: 3
                        color: "#BBDEFB"
                    ...
                    Rectangle {
                        width: addConnectorPairDialog.colPinB
                        height: parent.height - 4
                        anchors.verticalCenter: parent.verticalCenter
                        radius: 3
                        color: "#E8F5E9"
                    ...
                    Rectangle {
                        width: addConnectorPairDialog.colPortB
                        height: parent.height - 4
                        anchors.verticalCenter: parent.verticalCenter
                        radius: 3
                        color: "#C8E6C9"
                    ...
                    Item {
                        width: addConnectorPairDialog.colDel
                        height: parent.height
                    }
                }
            }
            ...
            Rectangle {
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        width: pinRowsList.width
                        height: 34
                        color: {
                            var isEmpty = String(
                                        model.pinNameA || "").trim() === ""
                                    && String(model.portMcuA
                                              || "").trim() === "" && String(
                                        model.pinNameB || "").trim() === ""
                                    && String(model.portMcuB
                                              || "").trim() === ""
                            if (isEmpty)
                                return "#FAFAFA"
                        ...
                        Row {
                            ...
                            QC.TextField {
                                ...
                                text: model.pinNameA || ""
                                horizontalAlignment: Text.AlignHCenter
                                placeholderText: index === pinRowsModel.count
                                                 - 1 ? qsTr("nhập...") : ""
                                background: Rectangle {
                                    radius: 3
                                    color: parent.activeFocus ? "#EBF5FB" : "transparent"
                                    border.color: parent.activeFocus ? "#1565C0" : "#EEE"
                                    border.width: parent.activeFocus ? 2 : 1
                                }
                                onTextChanged: {
                                    pinRowsModel.setProperty(index,
                                                             "pinNameA", text)
                                    addConnectorPairDialog._autoAddRow()
                                }
                            }
                            // Port MCU A
                            QC.TextField {
                            ...
                            QC.TextField {
                                ...
                                background: Rectangle {
                                    radius: 3
                                    color: parent.activeFocus ? "#EBF8E8" : "transparent"
                                    border.color: parent.activeFocus ? "#2E7D32" : "#EEE"
                                    border.width: parent.activeFocus ? 2 : 1
                                }
                                onTextChanged: {
                                    pinRowsModel.setProperty(index,
                            ...
                            Item {
                                ...
                                Rectangle {
                                    ...
                                    Text {
                                        ...
                                        text: "✕"
                                        font.pixelSize: 11
                                        color: delMa3.containsMouse ? "#D32F2F" : "#CFD8DC"
                                    }
                                    MouseArea {
                                        id: delMa3
                                        anchors.fill: parent
                                        ...
                                        onClicked: {
                                            if (pinRowsModel.count > 1)
                                                pinRowsModel.remove(index)
                                        }
                                    }
                                }
                            }
            ...
            RowLayout {
                ...
                Text {
                    ...
                    text: {
                        ...
                        for (var i = 0; i < pinRowsModel.count; i++) {
                            ...
                            if (String(r.pinNameA || "").trim() !== ""
                                    || String(r.portMcuA || "").trim() !== "")
                                validCount++
                        }
                        return validCount > 0 ? qsTr("📋 %1 cặp chân → %2 scripts").arg(
                                                    validCount).arg(
                                                    5 + validCount * 4) : ""
                    }
                }
                ...
                Rectangle {
                    ...
                    gradient: Gradient {
                        GradientStop {
                            position: 0.0
                            color: createBtnMa2.pressed ? "#0D47A1" : createBtnMa2.containsMouse ? "#1565C0" : "#1976D2"
                        }
                        GradientStop {
                            position: 1.0
                            color: createBtnMa2.pressed ? "#0A3A80" : createBtnMa2.containsMouse ? "#0D47A1" : "#1565C0"
                        }
                    }
                    Row {
                        anchors.centerIn: parent
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            ...
                            if (result.error) {
                                connPairError2.text = result.error
                                connPairError2.visible = true
                                return
                            }
                            connPairError2.visible = false
                            buildScriptsForConnectorPair(
                                        result.labelA, result.labelB,
                                        result.pinsA, result.pinsB,
                                        addConnectorPairDialog._insertAtIndex)
                            if (typeof window !== "undefined" && window.addLog)
                                window.addLog(
                                            "Sửa bài đo",
                                            "Chèn script mới: ➕ Thêm cặp cổng mới tại vị trí "
                                            + (addConnectorPairDialog._insertAtIndex + 1))
        ...
        function _parseConnectorPairInput() {
            ...
            if (!labelA)
                return {
                    "error": qsTr("Vui lòng nhập Nhãn cổng A")
                }
            if (!labelB)
                return {
                    "error": qsTr("Vui lòng nhập Nhãn cổng B")
                }
            ...
            if (pinsA.length === 0)
                return {
                    "error": qsTr("Vui lòng nhập ít nhất 1 cặp chân")
                }
            return {
                "labelA": labelA,
                "labelB": labelB,
