...
ApplicationWindow {
    ...
    Dialog {
        ...
        property bool enableWireResistance: true
        property bool enableContinuity: true
        property bool enableInsulation: true
        ...
        Connections {
            ...
            function onAllPacketsSent() {
                if (notificationDialog.visible && notificationDialog.isSending) {
                    ...
                    notificationDialog.mcuReady = false  // Chờ MCU gửi 0x06
                    notificationDialog.statusMessage = qsTr("✓ Đã gửi xong %1 bản tin. Chờ MCU sẵn sàng (0x06)...")
                        .arg(mcuSender.queuedPacketCount)
                    ackTimeoutTimer.stop()
                }
            }
            ...
            function onMcuReadyForTest() {
                if (notificationDialog.visible) {
                    console.log("[DEBUG] Dialog: MCU Ready (0x06) - cho phép Start Test")
                    console.log("[DEBUG] allPacketsSentDone=" + notificationDialog.allPacketsSentDone +
                                " isSending=" + notificationDialog.isSending +
                                " isPaused=" + notificationDialog.isPaused)
                    // MCU gửi 0x06 = đã nhận đủ packets và sẵn sàng đo
                    notificationDialog.allPacketsSentDone = true
                    notificationDialog.isSending = false
                    notificationDialog.mcuReady = true
                    notificationDialog.statusMessage = qsTr("✓ MCU sẵn sàng! Bấm 「Start Test」 để đo.")
                    ackTimeoutTimer.stop()
                }
            }
            ...
            function onMcuStartTestResponse() {
                if (notificationDialog.visible && notificationDialog.isMeasuring) {
                    // ⚠️ BỎ QUA 0x05 thừa khi đang đọc máy đo
                    // if (notificationDialog.isReadingInstrument) {
                    //     console.log("[DEBUG] ⚠️ BỎ QUA 0x05 - đang đọc máy đo cho script",
                    //                 notificationDialog.currentMeasurementIndex + 1)
                    //     return
                    // }

                    startTestTimeoutTimer.stop()
                    notificationDialog.startTestRetryCount = 0  // Reset retry
                    notificationDialog._measureStartTime = Date.now()  // ← GHI THỜI ĐIỂM BẮT ĐẦU ĐO
                //    notificationDialog.isReadingInstrument = true  // KHÓA - bắt đầu đọc
                    console.log("[DEBUG] Dialog: MCU relay ON (0x05) - đọc máy đo cho script",
                                notificationDialog.currentMeasurementIndex + 1, "/", notificationDialog.totalMeasurements)
                    notificationDialog.statusMessage = qsTr("Đang đọc máy đo... (%1/%2)")
                        .arg(notificationDialog.currentMeasurementIndex + 1).arg(notificationDialog.totalMeasurements)
                    // Đảm bảo delay đủ lâu cho máy đo ổn định sau khi chuyển relay
                    measurementReadTimer.interval = 50    ////////////////// thời gian chờ trước khi đọc máy đo (đã giảm xuống 50ms)
        ...
        Timer {
            ...
            onTriggered: {
                if (notificationDialog.currentMeasurementIndex < notificationDialog.validScriptsForPair.length) {
                    ...
                    // Máy đo chưa kết nối → DỪNG và báo lỗi
                    console.log("[MEASURE] LỖI: Máy đo chưa kết nối cho:", st)
                //    notificationDialog.isReadingInstrument = false
                    notificationDialog.isMeasuring = false
                    notificationDialog.statusMessage = qsTr("❌ Máy đo chưa kết nối! Loại: %1. Vui lòng kết nối máy đo rồi thử lại.").arg(st)
                    return
                }
        ...
        function _advanceToNextMeasurement() {
            ...
            if (currentMeasurementIndex >= totalMeasurements) {
                ...
                console.log("[MEASURE] Gửi Start Test cho script",
                            currentMeasurementIndex + 1, "/", totalMeasurements)
                statusMessage = qsTr("Gửi Start Test %1/%2...")
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    GridLayout {
                        ...
                        CheckBox {
                            id: wireResistanceCheck
                            checked: notificationDialog.enableWireResistance
                            text: qsTr("Điện trở dây dẫn (Keithley 2110)")
                            font.pixelSize: 12
                            enabled: !notificationDialog.isSending && !notificationDialog.allPacketsSentDone
                            onCheckedChanged: {
                                notificationDialog.enableWireResistance = checked
                            }
                        }
                        ...
                        CheckBox {
                            id: insulationCheck
                            checked: notificationDialog.enableInsulation
                            text: qsTr("Cách điện giữa các chân (Keithley 2110)")
                            font.pixelSize: 12
                            enabled: !notificationDialog.isSending && !notificationDialog.allPacketsSentDone
                            onCheckedChanged: {
                                notificationDialog.enableInsulation = checked
                            }
                        }
            ...
            Rectangle {
                ...
                color: notificationDialog.allMeasurementsDone ? "#e8f5e9"
                     : notificationDialog.isMeasuring ? "#e3f2fd"
                     : notificationDialog.allPacketsSentDone ? "#e8f5e9"
                     : notificationDialog.isSending ? "#fff3e0"
                     : "#f5f5f5"
                border.color: notificationDialog.allMeasurementsDone ? "#4CAF50"
                            : notificationDialog.isMeasuring ? "#2196F3"
                            : notificationDialog.allPacketsSentDone ? "#4CAF50"
                            : notificationDialog.isSending ? "#FF9800"
                            : "#ccc"
                radius: 4

                Text {
                    anchors.centerIn: parent
                    text: notificationDialog.statusMessage || qsTr("Sẵn sàng - Bấm 'Gửi' để gửi bản tin xuống MCU")
                    font.pixelSize: 13
                    color: notificationDialog.allMeasurementsDone ? "#2E7D32"
                         : notificationDialog.isMeasuring ? "#1565C0"
                         : notificationDialog.allPacketsSentDone ? "#2E7D32"
                         : notificationDialog.isSending ? "#E65100"
                         : "#666"
                    font.bold: notificationDialog.allMeasurementsDone || notificationDialog.allPacketsSentDone
                }
            }
            ...
            Rectangle {
                ...
                radius: 6
                color: notificationDialog.allMeasurementsDone ? "#E8F5E9"
                     : notificationDialog.isMeasuring ? "#FFF3E0"
                     : notificationDialog.allPacketsSentDone ? "#E3F2FD"
                     : notificationDialog.isSending ? "#FFF8E1"
                     : "#F3E5F5"
                border.color: notificationDialog.allMeasurementsDone ? "#4CAF50"
                            : notificationDialog.isMeasuring ? "#FF9800"
                            : notificationDialog.allPacketsSentDone ? "#2196F3"
                            : notificationDialog.isSending ? "#FFC107"
                            : "#9C27B0"
                border.width: 2
                ...
                ColumnLayout {
                    ...
                    Text {
                        ...
                        text: notificationDialog.allMeasurementsDone
                            ? "✅ Bước 4/4: Hoàn thành!"
                            : notificationDialog.isMeasuring
                            ? "⏳ Bước 3/4: Đang đo..."
                            : notificationDialog.allPacketsSentDone
                            ? "👉 Bước 3/4: Bấm 「Start Test」"
                            : notificationDialog.isSending
                            ? "⏳ Bước 2/4: Đang gửi bản tin..."
                            : "👉 Bước 1/4: Kết nối xong → Bấm 「Gửi」"
                        font.pixelSize: 14
                        font.bold: true
                        color: notificationDialog.allMeasurementsDone ? "#2E7D32"
                             : notificationDialog.allPacketsSentDone ? "#1565C0"
                             : "#6A1B9A"
                        wrapMode: Text.WordWrap
                    }
                    ...
                    Text {
                        ...
                        text: notificationDialog.allMeasurementsDone
                            ...
                            : notificationDialog.isMeasuring
                            ? qsTr("Đo tự động %1/%2 - vui lòng chờ...").arg(notificationDialog.currentMeasurementIndex).arg(notificationDialog.totalMeasurements)
                            : notificationDialog.allPacketsSentDone
                            ? qsTr("MCU đã nhận %1 bản tin. Bấm nút 「Start Test」 bên phải để bắt đầu đo.").arg(notificationDialog.totalMeasurements)
                            : notificationDialog.isSending
                            ? notificationDialog.statusMessage
                            : qsTr("Kết nối Đầu A (%1) và Đầu B (%2, rồi bấm 「Gửi」 để gửi bản tin xuống MCU.")
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    radius: 10
                    visible: !notificationDialog.allPacketsSentDone && !notificationDialog.isPaused
                ...
                Rectangle {
                    ...
                    Layout.preferredHeight: 40
                    radius: 10
                    visible: (notificationDialog.isSending || notificationDialog.allPacketsSentDone || notificationDialog.isPaused) && !notificationDialog.allMeasurementsDone
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            if (notificationDialog.isPaused) {
                                ...
                                if (typeof mcuSender !== "undefined" && mcuSender) { mcuSender.sendSimpleCommand(0x07) }
                                if (notificationDialog.pausedDuringPhase === "sending") {
                                    ...
                                    if (mcuSender) { mcuSender.sendSimpleCommand(0x04); startTestTimeoutTimer.restart() }
                                } else {
                                ...
                                if (typeof mcuSender !== "undefined" && mcuSender) { mcuSender.cancelQueue(); mcuSender.sendSimpleCommand(0x02) }
                ...
                Rectangle {
                    ...
                    opacity: (notificationDialog.mcuReady && !notificationDialog.startTestSent) ? 1.0 : 0.35
                    color: startBtnMa.pressed ? "#2E7D32" : startBtnMa.containsMouse ? "#388E3C" : "#4CAF50"
                    border.color: "#1B5E20"
                    border.width: 1

                    SequentialAnimation on opacity {
                        running: notificationDialog.mcuReady && !notificationDialog.startTestSent
                    ...
                    Row {
                        ...
                        spacing: 6
                        Text {
                            text: notificationDialog.isMeasuring ? "⏳" : notificationDialog.mcuReady ? "▶" : "⏳"
                            font.pixelSize: 16; anchors.verticalCenter: parent.verticalCenter; color: "#fff"
                        }
                        Text {
                            text: notificationDialog.isMeasuring ? qsTr("Đang đo...") : notificationDialog.mcuReady ? qsTr("Start Test") : qsTr("Chờ MCU...")
                            font.pixelSize: 14; font.bold: true; color: "#fff"
                            anchors.verticalCenter: parent.verticalCenter
                        }
                    }
                    MouseArea {
                        ...
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        enabled: notificationDialog.mcuReady && !notificationDialog.startTestSent && !notificationDialog.allMeasurementsDone
                        onClicked: {
                            console.log("[DEBUG] User bam 'Start Test'")
                            notificationDialog.startTestSent = true
                            addLog("Đo lường", "Bắt đầu đo - Bài: " + (selectedPlanName || "N/A") + " | SN: " + (mainContent.productSerialNumber || "N/A") + " | Tổng: " + notificationDialog.totalMeasurements + " phép đo")
                            notificationDialog.isMeasuring = true
                            notificationDialog.currentMeasurementIndex = 0
                            notificationDialog.statusMessage = qsTr("Gửi Start Test 1/%1...").arg(notificationDialog.totalMeasurements)
                            if (typeof mcuSender !== "undefined" && mcuSender) { mcuSender.sendSimpleCommand(0x04); startTestTimeoutTimer.restart() }
                        }
                    }
                }
        ...
        onStopRequested: {
            ...
            if (typeof mcuSender !== "undefined" && mcuSender) {
                ...
                mcuSender.sendSimpleCommand(0x03)
